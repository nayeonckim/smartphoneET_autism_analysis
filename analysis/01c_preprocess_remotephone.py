...
for ses in range(10):
    ses = str(ses + 1)
    sessionName = 'session' + ses
    
    # Output file to save processed ET data. 
    hdf_output_file = os.path.join(exp_folder,'remotephone', 'ET_RemotePhone_'+ sessionName +'.hdf5')
    ...
    clean_hdf_output_file = os.path.join(exp_folder,'remotephone', 'ET_RemotePhone_'+ sessionName +'_clean.hdf5')
    ...
    for fname_ii in subj_files:
        ...
        fname_ii = subj_indiv + '_allmovies_gaze_session' + ses
        
        if os.path.exists(os.path.join(exp_folder,subj,fname_ii + '.mat')):
            ...
            if not len(gaze_data_all) == 0:
                ...
                view_distance_default = 30.5 # phone viewing distance = 11-13 inches = 30.5cm
                ...
                for vid_ii in vids_this:
                    ...
                    for pii in range(len(calib_data[vid_ii])):
                        if len(calib_data[vid_ii][pii]) > 6:
                ...
                for vid_ii in vids_this:
                    ...
                    session_ID = '%s/%s'%(subj_indiv,vid_fname) 
                    if first_entry:
                        if compress_hdf:
    ...
    with open(os.path.join(stim_dir, 'stimname_map_' + sessionName + '.json'), 'w') as f:
